...
export default observer(function NotificationsDisplay() {
    const {
        notificationStore: { createHubConnection, clearNotifications, commentNotifications, joinNotifications, followNotifications }
    } = useStore();
    ...
    return (
        <Popup trigger={<Icon name="bell" size="large" />} hoverable>
            <Item.Group divided>{notifELements}</Item.Group>
        </Popup>
    );
});
