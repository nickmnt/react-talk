...
export default observer(function Channelnitial() {
    const {
        directStore: { createChannel },
        groupStore: { stopEditing, nextPhase, name, description, setName, setDescription }
    } = useStore();
        ...
        <div style={{ backgroundColor: 'white', height: '100%' }}>
            ...
            <Stack direction="column" spacing={2} sx={{ width: '100%', marginTop: '2rem' }} alignItems="center" justifyContent="center">
                ...
                <TextField
                    ...
                    value={description}
                    onChange={(target) => setDescription(target.currentTarget.value)}
                    placeholder="Description (optional)"
                    sx={{ fontSize: '1.6rem', width: '65%' }}
                    inputProps={{ style: { fontSize: '1.5rem' } }}
                    InputLabelProps={{ style: { fontSize: '1.5rem' } }}
                />
            </Stack>
