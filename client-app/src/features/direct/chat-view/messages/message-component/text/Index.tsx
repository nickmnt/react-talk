...
export default observer(function Text({ isMe, name, text, date, isDoubleTick, showImg, type, attachedImg, attachedVideo, isLocal, localBlob, message, goToMessage }: Props) {
    ...
    return (
        <Paper className={`text${isMe ? '--me' : '--other'}`} square elevation={6}>
            {replyTo && (
                <div style={{ width: '100%', marginLeft: '1rem', height: '3.5rem', display: 'flex', marginTop: '1rem', opacity: '.8', cursor: 'pointer' }} onClick={() => goToMessage(replyTo.id)}>
                    <div style={{ width: '.3rem', height: '100%', backgroundColor: '#007fff' }} />
                    <Stack direction="column" justifyContent="center" sx={{ margin: '0 1.5rem', fontSize: '1rem', height: '100%' }}>
            ...
            {message.forwardUsername && (
                <div style={{ height: '3.5rem', display: 'flex', marginTop: '1rem', opacity: '.8', color: '#007FFF' }}>
                    <Stack direction="column" justifyContent="center" sx={{ margin: '0 1rem', fontSize: '1rem', height: '100%' }}>
