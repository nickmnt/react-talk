...
export default observer(function MessageComponent({ message, onRightClick, goToMessage, selected, toggleSelected }: Props) {
    ...
    const bind = useLongPress(() => {
        setReady(false);
        if (selected.length === 0) toggleSelected(message);
        setTimeout(() => {
            setReady(true);
        }, 500);
    });
    ...
    const {
        userStore: { user },
        directStore: { currentChat, updateLastSeen }
    } = useStore();
    ...
    return (
        <>
            <div className="message" onContextMenu={onRightClick} ref={ref} {...bind} onClick={() => selected.length > 0 && ready && toggleSelected(message)}>
                {selected.length > 0 && (
                    <div className={`message__selectCircle ${isSelected && 'message__selectCircle--selected'}`}>
                ...
                <Text
                    ...
                    isDoubleTick={doubleTick}
                    showImg={showImg}
                    attachedImg={message.type === 1 ? message.url : ''}
