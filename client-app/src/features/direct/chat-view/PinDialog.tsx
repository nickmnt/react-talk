...
export default observer(function PinDialog({ open, onClose }: Props) {
    const {
        directStore: { currentChat, menuMsg, addPin }
    } = useStore();
        ...
        <Dialog onClose={handleClose} open={open} sx={{ minWidth: '50vw' }}>
            <Paper>
                <Stack sx={{ padding: '2rem' }}>
                    ...
                    <FormControlLabel
                        sx={{ margin: '1.5rem 0' }}
                        control={<Checkbox value={value} onChange={(e) => setValue(e.target.checked)} />}
                        label={`Also pin for ${currentChat.type === 0 ? currentChat?.displayName : 'other members'}`}
                    />
                    <Stack flexDirection="row" justifyContent="flex-end">
                        ...
                        <Button
                            variant="text"
                            onClick={() => {
                                addPin(currentChat.id, menuMsg?.id, value);
                                handleClose();
                            }}
                        >
                            Pin
